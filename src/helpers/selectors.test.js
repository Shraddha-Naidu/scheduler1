import { getAppointmentsForDay } from "helpers/selectors.test";
  ...
  days: [
    {
      id: 1,
      name: "Monday",
      appointments: [1, 2, 3, 6, 8],
      interviewers: [1, 2, 3, 4, 5]
    },
    {
      id: 2,
      name: "Tuesday",
      appointments: [4, 5, 7, 9, 10],
      interviewers: [6, 7, 9, 10, 11]
    }
  ],
  appointments: {
...
test("getInterviewersForDay returns an array containing the correct Interviewer objects", () => {
  ...
  expect(third).toEqual(state.interviewers["9"]);
  expect(fourth).toEqual(state.interviewers["10"]);
});
